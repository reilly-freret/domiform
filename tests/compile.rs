...
use domiform::compile::AdapterKind;
...
fn resolves_names_refs_and_capabilities() {
    ...
    assert!(matches!(
        cfg.adapters[zigbee].kind,
        AdapterKind::Zigbee2Mqtt { ref host, port, .. } if host == "127.0.0.1" && port == 1883
    ));
...
fn resolves_matter_adapter_url_and_endpoint() {
    ...
    assert!(matches!(
        cfg.adapters[thread].kind,
        AdapterKind::Matter { ref url } if url == "ws://127.0.0.1:5580/ws"
    ));

    // `endpoint` defaults to 1 when omitted, and carries through when set.
    let lamp = cfg.device_id("desk_lamp").unwrap();
    let ceiling = cfg.device_id("ceiling").unwrap();
    assert_eq!(cfg.device(lamp).unwrap().endpoint, 1);
    assert_eq!(cfg.device(ceiling).unwrap().endpoint, 2);
}
